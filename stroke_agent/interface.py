def render_stroke_agent():
    ...
    from stroke_agent.utils.database import (
        create_database,
        save_prediction,
        get_history
    )

    from stroke_agent.utils.pdf_report import generate_pdf
...
.stButton > button p, .stButton > button span {
    ...
    try:
        ...
        model = joblib.load(
            str(Path(__file__).resolve().parent / "stroke_model.pkl")
        )
    ...
    # Bind patient identity to the authenticated SMART CDSS account.
    current_user = st.session_state.get("current_user", {}) or {}
    logged_name = str(current_user.get("full_name") or current_user.get("username") or "Logged-in user").strip()
    try:
        logged_age = int(float(current_user.get("age") or 0))
    except (TypeError, ValueError):
        logged_age = 0

    if logged_age <= 0:
        raw_dob = str(current_user.get("date_of_birth") or "").strip()
        if raw_dob:
            for dob_format in ("%Y-%m-%d", "%d-%m-%Y", "%d/%m/%Y", "%m/%d/%Y"):
                try:
                    dob = datetime.strptime(raw_dob, dob_format).date()
                    today = datetime.now().date()
                    logged_age = today.year - dob.year - (
    ...
    # Keep the model's existing valid numeric fallback only for profiles where
    # neither age nor date of birth has been saved yet.
    logged_age = max(1, min(120, logged_age if logged_age > 0 else 45))
    logged_gender = str(current_user.get("gender") or "Male").strip()
    if logged_gender not in ["Male", "Female"]:
        logged_gender = "Male"
    ...
    with st.container(border=True):
        ...
        with row1[0]:
            patient_name = st.text_input(
                "Patient Name", value=logged_name, disabled=True,
                help="Linked to the currently logged-in account."
            )
        with row1[1]:
            age = st.number_input(
                "Age", min_value=1, max_value=120, value=logged_age, disabled=True
            )
        with row1[2]:
            gender_options = ["Male", "Female"]
            gender = st.selectbox(
                "Gender", gender_options,
                index=gender_options.index(logged_gender), disabled=True
            )
        with row1[3]:
    ...
    if st.button("Get Answer"):
        ...
        if user_question.strip():
            ...
            try:
                ...
                from stroke_agent.rag_mcp import ask_medical_agent
